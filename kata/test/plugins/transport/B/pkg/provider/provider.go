...
type transportBProvider struct {
...
var provider *transportBProvider
...
func (t *transportBProvider) Terminate() error {
	t.stopListener()
...
func BuildInfo() string {
	...
	log.L(ctx).Info("transportBProvider.BuildInfo")
	return ProviderName
}
...
func InitializeTransportProvider(socketAddress string, listenerDestination string) error {
	...
	log.L(ctx).Info("transportBProvider.InitializeTransportProvider")
	if provider != nil {
...
func (p *transportBProvider) createInstance(ctx context.Context, createInstanceRequest *pluginPB.CreateInstance) error {
	log.L(ctx).Infof("transportBProvider.createInstance, name: %s, destination: %s", createInstanceRequest.GetName(), createInstanceRequest.GetMessageDestination())
	listenerContext, stopListener := context.WithCancel(ctx)
